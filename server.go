...
type server struct {
	protobufs.UnimplementedCiciCacheServer
...
// Get 实现ciciCache server的Get接口
func (s *server) Get(ctx context.Context, req *protobufs.GetRequest) (*protobufs.GetResponse, error) {
	group, key := req.GetGroup(), req.GetKey()
	resp := &protobufs.GetResponse{}

	log.Printf("[cicicache_svr %s] Recv RPC Request - (%s)/(%s)", s.addr, group, key)
	if key == "" {
		return resp, fmt.Errorf("key is nil")
	}
	g := GetGroup(group)
	if g == nil {
...
func (s *server) Start() error {
	...
	protobufs.RegisterCiciCacheServer(grpcServer, s)

	// 注册服务至etcd
	go func() {
		// Register never return unless stop singnal received
		err := etcd.Register("cicicache", s.addr, s.stopSignal)
		if err != nil {
...
func (s *server) SetPeers(peersAddr ...string) {
	...
	for _, peerAddr := range peersAddr {
		...
		service := fmt.Sprintf("cicicache/%s", peerAddr)
